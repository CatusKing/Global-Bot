...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('promote')
        .setDescription('Promote a staff member and update the db.')
        .addUserOption(option => option.setName('member').setDescription('The person you want to promote.').setRequired(true))
        .addRoleOption(option => option.setName('position').setDescription('The position you want to promote the member to.').setRequired(true))
        ...
        try {
            ...
            for (let i = 0; i < data.staff.length; i++) {
                if (data.staff[i].id === member.id) {
                    ...
                    let content = `Promoted ${member} to ${role}`;
